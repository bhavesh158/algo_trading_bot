...
- Technical indicators (RSI, volatility, volume, trend alignment)
- Support/resistance proximity
- Candle pattern recognition
- LLM-powered market analysis (optional)
- News sentiment (optional)

Acts as a decision support filter — boosts or penalizes signal confidence.
"""
...
class AIAnalysis:
    """Multi-factor signal filter and confidence adjuster."""
    ...
    def __init__(self, config: dict[str, Any], event_bus: EventBus) -> None:
        ...
        weights = ai_cfg.get("factor_weights", {})
        ...
        # Initialize LLM + news if configured
    ...
    def adjust_confidence(self, signal: Signal) -> Signal:
        """Adjust signal confidence using multi-factor analysis."""
        ...
        # If LLM/news are disabled, redistribute their weight to technical
        ...
        if llm_active and news_active:
            ...
            adjustment = tech_score  # 100% technical
    ...
    def _check_trend_alignment(self, signal: Signal) -> float:
        """Boost if signal aligns with EMA trend."""
        df = self._market_data.get_dataframe(signal.symbol, "15m")
        if df.empty:
            return 0.0
    ...
    def _check_volatility_regime(self, signal: Signal) -> float:
        """Penalize during extreme volatility."""
        df = self._market_data.get_dataframe(signal.symbol, "15m")
        if df.empty or "atr" not in df.columns or len(df) < self._lookback:
            return 0.0

        atr = df["atr"].iloc[-1]
        mean_atr = df["atr"].iloc[-self._lookback:].mean()
        if pd.isna(atr) or pd.isna(mean_atr) or mean_atr == 0:
            return 0.0
    ...
    def _check_support_resistance(self, signal: Signal) -> float:
        """Boost if entry price is near support (BUY) or resistance (SELL)."""
        df = self._market_data.get_dataframe(signal.symbol, "15m")
    ...
    def _get_llm_adjustment(self, signal: Signal) -> float:
        ...
        try:
            ...
            # Gather indicators for LLM context
            indicators: dict[str, Any] = {
                "price": signal.entry_price,
                "side": signal.side.name,
                "strategy": signal.strategy_id,
            }
            for col in ["rsi", "adx", "atr", "ema_9", "ema_21", "bb_upper", "bb_lower"]:
    ...
    def _get_news_adjustment(self, signal: Signal) -> float:
        ...
        try:
            sentiment = self._news_sentiment.get_sentiment(signal.symbol, asset_type="crypto")

            # Convert sentiment (-1 to +1) to directional adjustment
            if signal.side == OrderSide.BUY:
                return sentiment * self._min_boost  # Positive news helps buys
            else:
                return -sentiment * self._min_boost  # Positive news hurts sells
        except Exception:
